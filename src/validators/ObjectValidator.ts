...
export default class ObjectValidator extends Validator {
	...
	private readonly rule_object?: { [property: string]: Validator }

	public constructor(rule_object?: { [property: string]: Validator }) {
		super()
	...
	public validate(data: any, reporter: Reporter): boolean {
		...
		for (const [rule_key, rule_value] of Object.entries(this.rule_object)) {
			const rule_rejects_undefined = !rule_value.validate(undefined, reporter.silence())

			if (!Object.keys(data).includes(rule_key) && rule_rejects_undefined) {
